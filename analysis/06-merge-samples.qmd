...
Below we plot the first two components of the PCA that we completed on the merged seurat object coloured by `Group`, `nCount_Spatial` and `nFeature_Spatial`.

::: {.panel-tabset}

## Group

@fig-pca-group-merged Plot PCA Group merged
...
elbow(seurat, vline1 = params$elbow_plot[[1]], vline2 = params$elbow_plot[[2]], title = "Elbow-Merged")
```

::: callout-note
Please take the number of PCs (e.g. `1:20`) you want as inferred from inspecting the elbow plot and rerun this workflow with the `elbow_range` parameter reset for your needs. See @tbl-params for more details. We advise to take a larger PC number than the exact elbow visualised.
:::
...
@fig-umap-chosen Plot UMAP merged Group
...
Below we plot the merged TSNE by `Group`, `nCount_Spatial`, and `nFeature_Spatial`.
...
list(
File = "seurat",
Link = "[seurat](../../output/06-merge-samples.rds)",
Description = "Merged Seurat object saved as rds object filtered and normalised with dimensionality reduction and clustering completed."
)
