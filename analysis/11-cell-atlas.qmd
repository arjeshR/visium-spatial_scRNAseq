...
params:
  ...
  r.memory:
    - 25 # number of GiBs required to run the SCT Transformation
  plot_width:
    - 6
  plot_height:
    - 5
...
list(
Parameter = "r.memory",
Value = params$r.memory,
Description = "Total Storage in Gigabytes required to optimise the SCT Transformation function."
)
)
...
```{r}
options(future.globals.maxSize = params$r.memory * 1024^3) 

atlas1 <- SCTransform(atlas, method = "glmGamPoi", ncells = params$ncells.sct, conserve.memory = TRUE,
...
if (params$vars.to.regress != FALSE) {
  ...
  atlas <- SCTransform(atlas, method = "glmGamPoi", ncells = params$ncells.sct, conserve.memory = TRUE,
                       variable.features.n = params$nfeatures.sct ,
                       verbose = FALSE, 
...
top.clusters <- head(VariableFeatures(seurat), params$top.features)
